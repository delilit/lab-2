...
- Взятый скрипт был изменён и доработан. Основное изменение это добавление return; в случае отсутсвия данных в dataset, что помогает изежать ошибок, что были в оригинальном коде, связанных с тем, что скрипт пытался получить данные которые ещё не были взяты из таблицы. В моём коде этой ошибки нет и программа работает без ошибок в логе. Также были измененны условия, так что теперь скрипт стал более оптимизированным, поскольку теперь, он проверяет значения statusStart и dataSet.Cound лишь раз. Также исправленна ошибка, из-за которой не вопсроизводился последний элемент, а также ошибки после воспроизведения всех элементов тоже не появляються.
```C#
